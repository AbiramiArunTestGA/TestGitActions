                                        ...
                                        if ($data.Tables[0].Rows.Count -gt 0)
                                        {
                                            ...
                                            foreach ($row in $data.Tables[0].Rows)
                                            {
                                                if ($row["InstructionName"] -eq $myInstName) #if instruction names match, then we see if there is any other change for the instruction
                                                {
                                                    ...
                                                    {
                                                        #There is a change in the instruction. So capture the old data before updating and maintain history of Instruction
                                                        InsertInstructionIntoHistoryTable -InstName $row["InstructionName"] -InstReadPayLoad $row["InstructionReadablePayload"] -InstDesc $row["InstructionDescription"] -InstType $row["InstructionType"] -InstTtlMin $row["InstructionTtlMinutes"] -InstTtlRespMin $row["InstructionResponseTtlMinutes"] -InstVer $row["InstructionVersion"] -InstAuth $row["InstructionAuthor"] -InstPayload $row["InstructionPayload"] -InstComments $row["InstructionComments"] -InstSchemaJson $row["InstructionSchemaJson"] -InstTaskGroups $row["InstructionTaskGroups"] -InstAggrJson $row["InstructionAggregateJson"] -InstSign $row["InstructionSignature"] -PathOfSQLDll $Path -Connection $oSQLiteDBConnection
                                                        Write-Log "History table Updated for $myInstName instruction." 1
                                                        #Update
...
function InsertInstructionIntoHistoryTable
{
    param
    (
        [String]$InstName,           #1
    ...
    try
    {
        ...
        #----------------- Begin INSERT Command --------------------------------------
        $oSQLiteDBCommand=$oSQLiteDBConnection.CreateCommand()
        $oSQLiteDBCommand.Commandtext="INSERT INTO InstructionsHistory (InstructionName, InstructionReadablePayload, InstructionDescription, InstructionType, InstructionTtlMinutes, InstructionResponseTtlMinutes, InstructionVersion,
                                        InstructionAuthor, InstructionPayload, InstructionComments, InstructionSchemaJson, InstructionTaskGroups, InstructionAggregationJson, InstructionSignature, InstructionCreatedDateTime, InstructionModifiedDateTime) VALUES (@InstructionName,
                                        @InstructionReadablePayload, @InstructionDescription, @InstructionType, @InstructionTtlMinutes, @InstructionResponseTtlMinutes, @InstructionVersion,
                                        @InstructionAuthor, @InstructionPayload, @InstructionComments, @InstructionSchemaJson, @InstructionTaskGroups, @InstructionAggregationJson, @InstructionSignature, @InstructionCreatedDateTime, @InstructionModifiedDateTime)"
        ...
        $oSQLiteDBCommand.Parameters.AddWithValue("InstructionSignature", $InstSign);                         #14
        $oSQLiteDBCommand.Parameters.AddWithValue("InstructionCreatedDateTime", $(Get-Date));                 #15
        $oSQLiteDBCommand.Parameters.AddWithValue("InstructionModifiedDateTime", $(Get-Date));                 #16
